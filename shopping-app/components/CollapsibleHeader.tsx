...
/** 1秒タイマーをヘッダー本体から分離し、親（サークル一覧）を再描画しない。 */
const ShoppingElapsed = React.memo(function ShoppingElapsed({
  startedAt,
}: {
  startedAt: string;
}) {
  ...
  return <Text style={styles.shoppingBadgeTime}> {elapsed}</Text>;
});
      ...
      <View style={styles.topRow}>
        ...
        {isShoppingMode && (
          <View style={styles.shoppingBadge}>
            ...
            {shoppingStartedAt ? <ShoppingElapsed startedAt={shoppingStartedAt} /> : null}
          </View>
        )}
