...
export default function CircleExpandedDetail({
  ...
  const memoRef = useRef(circle.memo);
  ...
  useEffect(() => {
    return () => {
      ...
      // Invalidate every in-flight query before this detail row unmounts.
  ...
  async function loadData() {
    ...
    setLoading(true);
    try {
      const [itemList, imageList] = await Promise.all([
      ...
      if (!mountedRef.current || !loadEpochGuardRef.current.isCurrent(epoch)) {
        return;
      }
      setItems(itemList);
      setImages(imageList);
      ...
      recordUiMetric("circle-expand-sql-count", snapshot.count);
      recordUiMetric("circle-expand-sql-elapsed-ms", snapshot.elapsedMs);
      if (mountedRef.current && loadEpochGuardRef.current.isCurrent(epoch)) {
        setLoading(false);
      }
    }
  ...
  async function handleReprocessFromPost() {
    ...
    const reprocessEpoch = loadEpochGuardRef.current.next();
    const requestedCircle = circleRef.current;
    ...
    try {
      ...
      if (
        !mountedRef.current ||
        !loadEpochGuardRef.current.isCurrent(reprocessEpoch)
      ) {
        return;
      }
      if (result.success) {
        ...
        await loadData();
        // memoも更新されているので親へ伝搬。リスト全体の再取得は行わない。
        const latestCircle = circleRef.current.id === requestedCircle.id
          ? circleRef.current
          : requestedCircle;
        const latestMemo = memoRef.current;
        const newMemo = latestMemo.includes(url)
          ? latestMemo
          : latestMemo
            ? `${latestMemo}\n${url}`
            : url;
        onCircleUpdated({ ...latestCircle, memo: newMemo });
