...
def _write_tiles(image_path: Path, size: int, overlap: int, max_count: int = 32) -> tuple[list[Path], list[tuple[int, int]]]:
    with Image.open(image_path) as image:
        ...
        if not origins:
            return [], []
        tile_dir = DEFAULT_OUTPUT_DIR / "tiles"
...
def main() -> int:
    ...
    for image_arg in args.image:
        ...
        try:
            ...
            if (
                _numeric_element_count(
                    [item for item in result.get("elements", []) if isinstance(item, dict)]
                ) < MIN_FULL_FRAME_NUMBERS
                and not args.no_tile_fallback
                and args.tile_size > 0
            ):
                ...
                if tiled:
                    # full-frameに少数でも有効な要素があれば捨てずに統合する。
