...
- **OCR（マップ座標自動推定）**: モバイルでは省略する。Unlimited OCRは6GB級モデルと
  CUDA/専用Python venvを必要とするため、デスクトップの責務とする。デスクトップ側で
  生成した `event.json` の `pin_x`/`pin_y`（0〜1正規化）をZIPへ含め、モバイルは
  それを表示・手動補正するだけにする。専用モデルやHFキャッシュをAPKへ埋め込まない。
  手動ピン配置は既実装でドラッグ対応済のため、OCRを省略してもデータ交換の契約は保てる。
