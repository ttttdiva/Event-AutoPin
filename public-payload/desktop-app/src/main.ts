...
function addPurchasedItemsFromEvent(data: any, index: Set<string>) {
  for (const circle of data?.circles ?? []) {
    const circleKeys = circlePurchaseKeys(circle);
    if (!circleKeys.length) continue;
    for (const item of circle.items ?? []) {
      if (getEffectiveItemStatus(item, circle) !== PURCHASE_STATUS.BOUGHT) {
        continue;
      }
      const itemKey = normalizePurchaseLookupKey(item?.name);
      if (!itemKey) continue;
      for (const circleKey of circleKeys) {
        index.add(purchasedItemKey(circleKey, itemKey));
      }
    }
  }
}
...
async function buildPurchasedItemIndex(
  ...
  for (const ev of eventList) {
    if (ev.slug === selectedSlug && selectedData) {
      addPurchasedItemsFromEvent(selectedData, next);
      continue;
    }
    try {
      const eventJsonPath = `${ev.dir}/event.json`.replace(/\\/g, "/");
      const res = await invokeBridgeJob<Record<string, unknown>>(
        "load_event_json",
      ...
      const data =
        res?.bridge && (res.bridge as any).status === "ok"
          ? (res.bridge as any).data
          : null;
      addPurchasedItemsFromEvent(data, next);
    } catch {
...
function renderCircleEditor() {
  ...
  // イベント情報行（行0）をスキップ: 最初の行のサークル名が空なら除外
  const startIdx =
    tableState.rows.length > 0 && !tableState.rows[0]["サークル名"] ? 1 : 0;
  ...
  const rowsHtml = filteredIndices
    .map((dispIdx) => {
  ...
  // ジャンル列（select）
  circleEditorEl
    .querySelectorAll<HTMLSelectElement>("select.genre-select")
    .forEach((sel) => {
      sel.addEventListener("change", () => {
        const row = Number(sel.dataset.row);
        const col = String(sel.dataset.col);
        tableState.rows[row][col] = sel.value;
        saveNow();
        // circle_masterにもジャンルを書き込み
        const rowData = tableState.rows[row];
        const cName = (rowData["サークル名"] || "").trim();
        const penname = (rowData["ペンネーム"] || "").trim();
        if (cName) setCircleMasterGenre(cName, penname, sel.value);
      });
      sel.addEventListener("click", (e) => e.stopPropagation());
    });

  circleEditorEl.querySelectorAll("input[data-row][data-col]").forEach((el) => {
    el.addEventListener("change", (e) => {
      const t = e.target as HTMLInputElement;
      const row = Number(t.dataset.row);
      const col = String(t.dataset.col);
      tableState.rows[row][col] = t.value;
      saveNow();
    });
  });
...
(
  ...
  try {
    ...
    // Ignore stale server cleanup failures; start_file_server will report real errors.
  }

  resultEl.textContent = "全イベント同期ZIPを作成中...";
  const zipRes = await runJob("create_mobile_full_sync_zip", {
...
function updateActiveEventDisplay(meta: EventMeta) {
  ...
  if (crawlEl)
    crawlEl.innerHTML = `<strong>対象イベント:</strong> ${escapeHtml(info)}${shoppingHtml}`;

  const mobileEl = document.getElementById("mobileActiveEvent");
  if (mobileEl)
    mobileEl.innerHTML = `<strong>対象イベント:</strong> ${escapeHtml(info)}${shoppingHtml}`;
}
...
function renderSidebar() {
  ...
  listEl.innerHTML = eventList
    .map((ev) => {
      ...
      return `
      <div class="event-card ${isActive ? "active" : ""} ${isCompleted ? "completed" : ""}" data-slug="${escapeHtml(ev.slug)}" tabindex="0" title="${cardTitleHtml}">
  ...
  listEl.querySelectorAll<HTMLDivElement>(".event-card").forEach((card) => {
    // シングルクリック: イベント選択（ダブルクリックと区別するため遅延）
    card.addEventListener("click", () => {
      ...
      clickTimer = window.setTimeout(() => {
        ...
        if (slug && slug !== activeEventSlug) {
          selectEvent(slug);
        } else if (slug && slug === activeEventSlug) {
          // 既に選択中ならフォーカスだけ当てる（F2/Delete用）
          card.focus();
        }
      }, 200);
    });
...
async function selectEventLocked(
  ...
  try {
    ...
    if (metaRes.found) nextMeta = metaRes.meta;
  } catch {
    /* ignore */
  }
  if (isStale()) return false;
  ...
  try {
    ...
    if (res?.bridge && (res.bridge as any).status === "ok") {
      nextEventJsonData = ((res.bridge as any).data || null) as EventJsonData | null;
      loadSucceeded = true;
    }
  } catch {
