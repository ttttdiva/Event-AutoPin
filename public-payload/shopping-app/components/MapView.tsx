...
import { PURCHASE_STATUS } from "@/lib/types";
...
const MapViewComponent = forwardRef<MapViewHandle, MapViewProps>(
  function MapViewComponent(
    ...
    const pinsForMap = useMemo(() => {
      let result = circles.filter((c) => c.pinX != null && c.pinY != null);
      if (currentMap) {
        result = result.filter(
          (c) =>
            c.mapNumber === currentMap.mapNumber ||
            c.mapNumber == null ||
            c.mapNumber === 0,
        );
      }
      // M4: 完了/×/見送りサークルのピンはデフォルトで非表示
      if (parentStatusFilter == null) {
        result = result.filter(
          (c) => c.purchaseStatus === PURCHASE_STATUS.NOT_YET,
        );
      }
      // マップ独自のフィルター
      if (colorFilter.size > 0) {
        result = result.filter((c) => colorFilter.has(c.priorityColor));
      }
      // M2: 親のフィルターをマップにも適用
      if (parentStatusFilter != null) {
        result = result.filter((c) => c.purchaseStatus === parentStatusFilter);
      }
      if (parentPriorityFilter && parentPriorityFilter.size > 0) {
        result = result.filter((c) =>
          parentPriorityFilter.has(c.priorityColor),
        );
      }
      if (parentHallFilter) {
        result = result.filter((c) => c.hall === parentHallFilter);
      }
      if (parentCatalogPostOnly) {
        result = result.filter((c) => c.hasCatalogPost);
      }
      if (parentHideSkipped) {
      ...
      if (parentSearchQuery && parentSearchQuery.trim()) {
        ...
        result = result.filter(
          (c) => {
            const itemMatches = (parentSearchTextMap?.get(c.id) ?? []).some(
              (text) => text.toLowerCase().includes(q),
            );
            if (parentGlobalSearchEnabled) {
              return (
                  ...
                  <DraggablePin
                    ...
                    onLongPress={() => {
                      if (onPinRemove) {
                        Alert.alert(
                          "ピン削除",
                          `「${circle.name}」のピンを削除しますか？`,
                          [
                            { text: "キャンセル", style: "cancel" },
                            {
                              text: "削除",
                              style: "destructive",
                              onPress: () => onPinRemove(circle),
                            },
                          ],
                        );
                      }
                    ...
                    onDragEnd={
                      onPinMove
                        ? (tx, ty) => {
                            // ドラッグ距離をcurrentScaleで割って、正規化座標のデルタを計算
                            const deltaNormX =
                              tx / (currentScale * displaySize.w);
                            const deltaNormY =
                              ty / (currentScale * displaySize.h);
                            const newNormX = Math.max(
                              0,
                              Math.min(1, (circle.pinX ?? 0) + deltaNormX),
                            );
                            const newNormY = Math.max(
                              0,
                              Math.min(1, (circle.pinY ?? 0) + deltaNormY),
                            );
