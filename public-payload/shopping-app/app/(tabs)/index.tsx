...
const SKIPPED_STATUS_COLOR = PURCHASE_STATUS_LABELS[PURCHASE_STATUS.SKIPPED].color;
...
export default function EventListScreen() {
  ...
  const { refreshKey, refresh, setCurrentEventId } = useEvent();
  ...
  async function loadData() {
    ...
    try {
      ...
      for (const event of eventList) {
        const s = await getEventStats(event.id);
        withStats.push({ ...event, ...s });
      }
      setEvents(withStats);
    } catch (e) {
  ...
  // イベントタップ → サークルリスト画面に遷移
  function handleEventPress(event: EventWithStats) {
    setCurrentEventId(event.id);
    router.push(`/event/${event.id}` as any);
  }

  // イベント長押し → カスタムモーダル表示
  function handleEventLongPress(event: EventWithStats) {
  ...
  async function handleAction(action: string) {
    ...
    switch (action) {
      ...
      case "toggleCompleted":
        await toggleEventCompleted(ev.id);
        loadData();
        break;
      case "memo":
        setMemoText(ev.memo || "");
        ...
        if (ev.eventImageFilename) {
          Alert.alert("イベント画像", "", [
            { text: "キャンセル", style: "cancel" },
            {
              text: "画像を変更",
              onPress: async () => {
                await pickAndSaveEventImage(ev.id);
                loadData();
              },
            },
            {
  ...
  function renderEventItem({ item }: { item: EventWithStats }) {
    const isShoppingActive =
      item.shoppingStartedAt != null && item.shoppingEndedAt == null;
    const progress =
      item.totalCircles > 0
        ? (item.boughtCircles / item.totalCircles) * 100
        : 0;
    const couldntProgress =
      item.totalCircles > 0
        ? (item.couldntBuyCircles / item.totalCircles) * 100
        : 0;
    const skippedProgress =
      item.totalCircles > 0
        ? (item.skippedCircles / item.totalCircles) * 100
        : 0;

    const eventImagePath = item.eventImageFilename
      ? item.eventImageFilename.startsWith("file://") ||
        item.eventImageFilename.startsWith("/")
        ? item.eventImageFilename
        : null
      : null;
    ...
    return (
      <Pressable
        style={[
          ...
          item.completed && styles.eventCardCompleted,
        ]}
        ...
        <View style={styles.eventRow}>
          ...
          {eventImagePath ? (
            <Image
              ...
              recyclingKey={`event-img-${item.id}`}
              transition={100}
            />
          ) : (
            <View
              style={[
                ...
                {
                  backgroundColor:
                    effectiveScheme === "dark" ? "#333" : "#e8e8e8",
                },
              ]}
            >
              <Text
                style={[
                  styles.eventNoImageText,
          ...
          <View style={styles.eventContent}>
            ...
            <View style={styles.eventProgressRow}>
                  ...
                  <>
                    ...
                    <View
                      style={[
                        styles.eventProgressFill,
                        {
                          backgroundColor: SKIPPED_STATUS_COLOR,
                          width: `${skippedProgress}%`,
                          position: "absolute",
                          left: `${progress + couldntProgress}%`,
                        },
                      ]}
                    />
                  </>
              ...
              <Text
                style={[
                  styles.eventProgressText,
                  { color: colors.textSecondary },
                ]}
              >
                {item.boughtCircles}/{item.totalCircles}
              </Text>
            </View>
          </View>
        </View>
